...
{   int elec;
    ...
    printf("\n\nz1: %+.4lf%+.4lfi = %.4lf e^(%+.4lf) \n",creal(z1),cimag(z1),cabs(z1),carg(z1)*180.0/pi);
    printf("\n\nz2: %+.4lf%+.4lfi = %.4lf e^(%+.4lf) \n\n",creal(z2),cimag(z2),cabs(z2),carg(z2)*180.0/pi);
...
double complex impRec()
{
    ...
    scanf("%lf", &Iimp);
    printf("Impedancia inicializada: %.2f%+.2fi\t = %.2f e^(%+.2fi)\n\n",Rimp,Iimp,cabs(Rimp+Iimp*I),carg(Rimp+Iimp*I)*180.0/pi);
    return Rimp+Iimp*I;
}
...
double complex impPol()
{
    ...
    scanf("%lf", &Iimp);
    Iimp = Iimp*pi/180.0; // paso el ang sexag a rad
    printf("Impedancia inicializada: %.2f%+.2fi\t = %.2fe^(%+.2fi)\n\n",creal(Rimp*cexp(Iimp*I)),cimag(Rimp*cexp(Iimp*I)),Rimp,Iimp*180.0/pi);
