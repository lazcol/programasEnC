...
int main() {
    ...
    int term = 75;          // cantidad de términos dependientes de t
    ...
    for(t=0.0 ; t<P*T ; t=t+P*T/largo) {                // primer for: cada espacio de tiempo
        ...
        for(n = 1; n <= term; n++) {                    // segundo for : para cada espacio de tiempo, genera los términos solicitados
            ...
            h1 = h1 -2*A/T/n/w * sin(n*w*t) ;             // Esta línea es la Serie Trigonométrica de Fourier

            h2 = h2 +2*A/T/n/w*I*cexp(I*n*w*t) ;       // Esta línea es la Serie Compleja de Fourier
        }
        putpixel(t*Ampli, getmaxy()/2 - Ampli*(h1+A/2), 15); // pone un punto en la gráfica con las coordenadas halladas (Serie Trigonométrica)
        putpixel(t*Ampli, getmaxy()/2 - Ampli*(creal(h2)+A/2), 5);  // pone un punto en la gráfica con las coordenadas halladas (Serie Compleja)
